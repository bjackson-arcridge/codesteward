...
CodeSteward is implemented around a portable core:

- `codesteward` CLI for DR retrieval, candidate creation, status validation, tags, and bootstrap.
...
CodeSteward is a continuous learning system in the practical product sense, not in the model-training sense. It does not fine-tune the model, update weights, or permanently change the underlying LLM. It gives the agent a durable project memory of the engineer's decisions.
...
A DR is broader than an ADR (Architectural Decision Record). CodeSteward records architectural decisions, implementation patterns, dependency choices, organizational patterns, testing approaches, error-handling strategy, and other choices an agent should not silently reinvent.
...
Candidates appear in CodeSteward's Candidate Inbox. The engineer can:
...
On disk, candidates live under `.codesteward/drs/candidates/`, not in a separate top-level `.codesteward/candidates/` directory. Acceptance moves a candidate into `.codesteward/drs/accepted/`; rejection or supersession moves it into the corresponding DR lifecycle folder.
...
2. Create an initial proposal using accepted DR context from `codesteward dr retrieve`.
...
7. Leave proposed candidates in CodeSteward for engineer review.
...
6. Run `codesteward status` when the DR store changes.
...
`codesteward init` appends this rule to the project's agent instruction file, such as `AGENTS.md`, `AGENT.md`, or `CLAUDE.md`:
...
This rule applies broadly. A user message during design, implementation, review, debugging, or cleanup may contain a durable decision. CodeSteward should bias toward asking "is this reusable guidance?" and recording a candidate when the answer might be yes.

---

## Portability Model

CodeSteward's portable core is:
...
The VS Code extension is the human stewardship surface. It is not the source of truth; the CLI and `.codesteward/` store are.
...
For the first version, CodeSteward does not need configurable agency levels. The default policy is deliberately simple:
...
- Leave candidate DRs in CodeSteward for engineer review before they become precedent.
...
**Amazon Q's AI-DLC** proves adaptive rigor works: deeper checkpoints for more complex tasks, clarifying questions for ambiguity, and traceable review history. CodeSteward differs by centering engineer authority rather than agent-directed process.

**Knowledge-agent patterns** show the value of logging assumptions and feeding them back into future work. CodeSteward makes the engineer's decision, not the agent's assumption, the durable unit.

**ADR-driven development** provides the strongest conceptual base. CodeSteward broadens ADRs into DRs because agents need guidance on implementation patterns, testing strategy, dependency choices, and error-handling decisions too.

**Human-in-the-loop research such as HULA** supports the claim that human guidance matters more as tasks become more complex. CodeSteward pushes beyond human-in-the-loop toward human-in-control.
...
- [CLI_SPEC.md](CLI_SPEC.md) defines the `codesteward` CLI surface.
