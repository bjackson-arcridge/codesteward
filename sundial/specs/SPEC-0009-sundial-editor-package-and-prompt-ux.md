...
1. Add an independently packaged `packages/editor` VS Code extension, published as `arcridge.sundial-editor`, now at version `0.2.3` after the command-mode and keyboard-only composer refinements. Add the package to the root npm workspaces, package/test helpers, and lockfile without changing the existing governance extension or the CLI. Give the package its own manifest, README, TypeScript configurations, esbuild host/browser builds, copied Sundial icon, and VSIX packaging script. It depends neither on the governance extension nor on the Sundial CLI. Its collaboration view targets VS Code's right-hand Secondary Sidebar, the workspace normally used for LLM chat panes.

2. Implement a small extension-host command and pure parser for an entire source line. A `%` in column zero enters Sundial command mode; supported lines are `%Q`, `%1:F`, `%1:W`, `%1:R`, `%2:C`, and `%0:T`, with an optional trailing `@G` modifier such as `%1:F @G`. Trailing horizontal whitespace is permitted, but leading whitespace, source text, legacy presets, unrecognised presets, and malformed modifiers are rejected without changing the document. The numeric portion remains opaque preset metadata rather than an agent identifier in this slice.
...
None for this slice. The numeric portions in the established presets are deliberately opaque metadata until the agent-routing slice defines their delivery semantics; `@G` is accepted as an optional trailing modifier and is UI-only until command annotation persistence exists.
